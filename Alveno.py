...
def banner():
	print(f'''\t{asu}    	
			{m}•{k}•{h}•{sir} Author : DOCTOR_EXPERT {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	print('>> 2. Crack With Followers ')
	print('>> 3. Crack ID Group  ')
	print('>> 4. Crack  With Combo	')
	print('>> 5. Crack Random  ')
	print('>> 0. Exit       ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> HOW MUCH ID YOU WANT CRACK? : '))
	except ValueError:
